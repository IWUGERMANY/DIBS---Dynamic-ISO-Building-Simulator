...
class DIBS:
    ...
    def calculate_result_of_one_building(self, path: str, user_arguments: List):
        ...
        with Progress() as progress:
            ...
            start_time = time.time()
            csv_file_name = simulator.datasource.result_of_all_hours_to_csv(
    ...
    def save_results_of_all_buildings_in_excel_parallel_using_thread_executor(
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(
                    self.datasource.result_of_all_hours_to_excel,
                    result[index],
                    building,
                )
                for index, building in enumerate(buildings)
