...
logging.info(building_data.dtypes)
...
for iteration, i_gebaeudeparameter in enumerate(namedlist_of_buildings):
    ...
    BuildingInstance = Building(
        scr_gebaeude_id=i_gebaeudeparameter.scr_gebaeude_id,
        ...
        heating_supply_system=getattr(
            supply_system, i_gebaeudeparameter.heating_supply_system
        ),
        cooling_supply_system=getattr(
            supply_system, i_gebaeudeparameter.cooling_supply_system
        ),
        heating_emission_system=getattr(
            emission_system, i_gebaeudeparameter.heating_emission_system
        ),
        cooling_emission_system=getattr(
            emission_system, i_gebaeudeparameter.cooling_emission_system
        ),
    )
    ...
    if (i_gebaeudeparameter.energy_ref_area == -8) | (
    ...
    if weather_period == "2007-2021":
        building_location = Location(
            epwfile_path=os.path.join(
                mainPath,
                "auxiliary/weather_data/weather_data_TMYx_2007_2021",
                epw_filename,
            )
        )
    else:
        building_location = Location(
            epwfile_path=os.path.join(
                mainPath, "auxiliary/weather_data", epw_filename)
        )
    ...
    gains_from_group_values = (
        mid_values  # Choose here here between low, mid or max values
    )
    gain_per_person, appliance_gains, typ_norm = normReader.getGains(
        BuildingInstance.hk_geb,
        BuildingInstance.uk_geb,
        profile_from_norm,
        gains_from_group_values,
    )
    ...
    usage_from_norm = sia
    usage_start, usage_end = normReader.getUsagetime(
        BuildingInstance.hk_geb, BuildingInstance.uk_geb, usage_from_norm
    )

    # Read specific occupancy schedule
    # Assignments see Excel/CSV-File in /auxiliary/occupancy_schedules/
    occupancy_schedule, schedule_name = scheduleReader.getSchedule(
        BuildingInstance.hk_geb, BuildingInstance.uk_geb
    )

    TEK_dhw, TEK_name = TEKReader.getTEK(
        BuildingInstance.hk_geb, BuildingInstance.uk_geb
    )  # TEK_dhw in kWh/m2*a
    # print(TEK_name)
    # print(TEK_dhw)
    ...
    TEK_dhw_per_Occupancy_Full_Usage_Hour = (
        TEK_dhw / Occupancy_Full_Usage_Hours
    )  # in kWh/m2*h
    ...
    for hour in range(8760):
        ...
        Altitude, Azimuth = building_location.calc_sun_position(
            latitude_deg=latitude_station,
            longitude_deg=longitude_station,
            year=building_location.weather_data["year"][hour],
            hoy=hour,
        )

        # Calculate H_ve_adj, See building_physics for details
        BuildingInstance.h_ve_adj = BuildingInstance.calc_h_ve_adj(
        ...
        SouthWindow.calc_solar_gains(
            ...
            sun_azimuth=Azimuth,
            normal_direct_radiation=building_location.weather_data["dirnorrad_Whm2"][
                hour
            ],
            horizontal_diffuse_radiation=building_location.weather_data[
                "difhorrad_Whm2"
            ][hour],
            t_air=t_air,
            hour=hour,
        ...
        SouthWindow.calc_illuminance(
            sun_altitude=Altitude,
            sun_azimuth=Azimuth,
            normal_direct_illuminance=building_location.weather_data["dirnorillum_lux"][
                hour
            ],
            horizontal_diffuse_illuminance=building_location.weather_data[
                "difhorillum_lux"
            ][hour],
        )

        EastWindow.calc_solar_gains(
            sun_altitude=Altitude,
            sun_azimuth=Azimuth,
            normal_direct_radiation=building_location.weather_data["dirnorrad_Whm2"][
                hour
        ...
        occupancy = (
            occupancy_schedule.loc[hour, "People"] *
            BuildingInstance.max_occupancy
        )

        # Calculate the lighting of the building for the time step
        BuildingInstance.solve_building_lighting(
        ...
        internal_gains = (
            ...
            + appliance_gains
            * occupancy_schedule.loc[hour, "Appliances"]
            * BuildingInstance.energy_ref_area
        ...
        BuildingInstance.solve_building_energy(
        ...
        if (
            i_gebaeudeparameter.dhw_system != "NoDHW"
            and i_gebaeudeparameter.dhw_system != " -"
        ):
            hotwaterdemand = (
                occupancy_schedule.loc[hour, "People"]
                * TEK_dhw_per_Occupancy_Full_Usage_Hour
            ...
            if BuildingInstance.heating_demand > 0:  # catch devision by zero error
                hotwaterenergy = hotwaterdemand * (
                    BuildingInstance.heating_energy / BuildingInstance.heating_demand
                )
            else:
                hotwaterenergy = hotwaterdemand
            ...
            if (i_gebaeudeparameter.dhw_system == "DecentralElectricDHW") or (
                (
                    (i_gebaeudeparameter.dhw_system == "CentralHeating")
                    | (i_gebaeudeparameter.dhw_system == "CentralDHW")
                )
                and (
                    (i_gebaeudeparameter.heating_supply_system == "HeatPumpAirSource")
                    | (
                        i_gebaeudeparameter.heating_supply_system
        ...
        Heating_Sys_Electricity.append(
            BuildingInstance.heating_sys_electricity)
        ...
        Cooling_Sys_Electricity.append(
            BuildingInstance.cooling_sys_electricity)
    ...
    Heating_Sys_Electricity_sum = hourlyResults.Heating_Sys_Electricity.sum() / \
        1000
    ...
    Cooling_Sys_Electricity_sum = hourlyResults.Cooling_Sys_Electricity.sum() / \
        1000
    ...
    HotWater_Sys_Electricity_sum = hourlyResults.HotWater_Sys_Electricity.sum() / \
        1000
    ...
    Appliance_gains_elt_demand_sum = (
        hourlyResults.Appliance_gains_elt_demand.sum() / 1000
    )
    ...
    if (
        i_gebaeudeparameter.heating_supply_system == "BiogasBoilerCondensingBefore95"
        ...
        (i_gebaeudeparameter.heating_supply_system == "OilBoilerStandardBefore86")
        ...
        (i_gebaeudeparameter.heating_supply_system == "LGasBoilerLowTempBefore95")
        ...
        (i_gebaeudeparameter.heating_supply_system == "GasBoilerStandardBefore86")
        ...
        (i_gebaeudeparameter.heating_supply_system == "WoodChipSolidFuelBoiler")
    ...
    # HEATING
    # GHG-Faktor Heating
    f_GHG = GWP_PE_Factors.loc[
        GWP_PE_Factors["Energy Carrier"] == Fuel_Type,
        "GWP spezific to heating value GEG [g/kWh]",
    ]
    f_GHG = f_GHG.iloc[0]

    # PE-Faktor Heating
    f_PE = GWP_PE_Factors.loc[
        GWP_PE_Factors["Energy Carrier"] == Fuel_Type, "Primary Energy Factor GEG   [-]"
    ]
    f_PE = f_PE.iloc[0]

    # Umrechnungsfaktor von Brennwert (Hs) zu Heizwert (Hi) einlesen
    f_Hs_Hi = GWP_PE_Factors.loc[
        GWP_PE_Factors["Energy Carrier"] == Fuel_Type,
    ...
    if Heating_Sys_Electricity_sum > 0:
        Heating_Sys_Electricity_Hi_sum = (
            Heating_Sys_Electricity_sum / f_Hs_Hi
        )  # for kWhHi Final Energy Demand
        Heating_Sys_Carbon_sum = (
            Heating_Sys_Electricity_Hi_sum * f_GHG
        ) / 1000  # for kg CO2eq
        Heating_Sys_PE_sum = (
            Heating_Sys_Electricity_Hi_sum * f_PE
        )  # for kWh Primary Energy Demand
    else:
        Heating_Sys_Fossils_Hi_sum = (
            Heating_Sys_Fossils_sum / f_Hs_Hi
        )  # for kWhHi Final Energy Demand
        Heating_Sys_Carbon_sum = (
            Heating_Sys_Fossils_Hi_sum * f_GHG
        ) / 1000  # for kg CO2eq
        Heating_Sys_PE_sum = (
            Heating_Sys_Fossils_Hi_sum * f_PE
        )  # for kWh Primary Energy Demand
    ...
    if HotWater_Sys_Electricity_sum > 0:
        HotWater_Sys_Electricity_Hi_sum = (
            HotWater_Sys_Electricity_sum / f_Hs_Hi
        )  # for kWhHi Final Energy Demand
        HotWater_Sys_PE_sum = (
            HotWater_Sys_Electricity_Hi_sum * f_PE
        )  # for kWh Primary Energy Demand
        HotWater_Sys_Carbon_sum = (
            HotWater_Sys_Electricity_Hi_sum * f_GHG
        ) / 1000  # for kg CO2eq
    else:
        HotWater_Sys_Fossils_Hi_sum = HotWater_Sys_Fossils_sum / f_Hs_Hi
        HotWater_Sys_PE_sum = (
            HotWater_Sys_Fossils_Hi_sum * f_PE
        )  # for kWh Primary Energy Demand
        HotWater_Sys_Carbon_sum = (
            HotWater_Sys_Fossils_Hi_sum * f_GHG
        ) / 1000  # for kg CO2eq

    HotWaterEnergy_Hi_sum = (
        HotWater_Sys_Electricity_Hi_sum + HotWater_Sys_Fossils_Hi_sum
    )
    ...
    if (
        (i_gebaeudeparameter.cooling_supply_system == "AirCooledPistonScroll")
        ...
        print(
            ...
            i_gebaeudeparameter.cooling_supply_system,
        )
    ...
    if Cooling_Sys_Electricity_sum > 0:
        Cooling_Sys_Electricity_Hi_sum = (
            Cooling_Sys_Electricity_sum / f_Hs_Hi
        )  # for kWhHi Final Energy Demand
        Cooling_Sys_Carbon_sum = (
            Cooling_Sys_Electricity_Hi_sum * f_GHG
        ) / 1000  # for kg CO2eq
        Cooling_Sys_PE_sum = (
            Cooling_Sys_Electricity_Hi_sum * f_PE
        )  # for kWh Primary Energy Demand
    else:
        Cooling_Sys_Fossils_Hi_sum = (
            Cooling_Sys_Fossils_sum / f_Hs_Hi
        )  # for kWhHi Final Energy Demand
        Cooling_Sys_Carbon_sum = (
            Cooling_Sys_Fossils_Hi_sum * f_GHG
        ) / 1000  # for kg CO2eq
        Cooling_Sys_PE_sum = (
            Cooling_Sys_Fossils_Hi_sum * f_PE
        )  # for kWh Primary Energy Demand
    ...
    LightingDemand_Hi_sum = (
        LightingDemand_sum / f_Hs_Hi
    )  # for kWhHi Final Energy Demand
    LightingDemand_Carbon_sum = (
        LightingDemand_Hi_sum * f_GHG) / 1000  # for kg CO2eq
    LightingDemand_PE_sum = (
        LightingDemand_Hi_sum * f_PE
    )  # for kWhHs Primary Energy Demand

    # electrical energy for Appliances
    Appliance_gains_demand_Hi_sum = (
        Appliance_gains_elt_demand_sum / f_Hs_Hi
    )  # for kWhHi Final Energy Demand
    ...
    print(
        "HeatingEnergy [kwh/m2]:", HeatingEnergy_sum /
        BuildingInstance.energy_ref_area
    )
    ...
    print(
        "CoolingEnergy [kwh/m2]:", CoolingEnergy_sum /
        BuildingInstance.energy_ref_area
    )
    ...
    print(
        "HotWaterEnergy [kwh/m2]:",
        HotWaterEnergy_sum / BuildingInstance.energy_ref_area,
    )
    ...
    annualResults_summary_temp = pd.DataFrame(
        {
            ...
            "HotWater_Sys_GWP [kg/m2]": HotWater_Sys_Carbon_sum
            / BuildingInstance.energy_ref_area,
            "HotWater_Sys_PE [kWh]": HotWater_Sys_PE_sum,
            "HotWater_Sys_PE [kWh/m2]": HotWater_Sys_PE_sum
            / BuildingInstance.energy_ref_area,
            "ElectricityDemandTotal [kWh]": Heating_Sys_Electricity_sum
            + HotWater_Sys_Electricity_sum
            + Cooling_Sys_Electricity_sum
            + LightingDemand_sum
            + Appliance_gains_elt_demand_sum,
            "ElectricityDemandTotal [kwh/m2]": (
            ...
            / BuildingInstance.energy_ref_area,
            "FossilsDemandTotal [kWh]": Heating_Sys_Fossils_sum
            + Cooling_Sys_Fossils_sum,
            "FossilsDemandTotal [kwh/m2]": (
                Heating_Sys_Fossils_sum + Cooling_Sys_Fossils_sum
            )
            / BuildingInstance.energy_ref_area,
            "LightingDemand [kWh]": LightingDemand_sum,
            "LightingDemand_GWP [kg]": LightingDemand_Carbon_sum,
            "LightingDemand_GWP [kg/m2]": LightingDemand_Carbon_sum
            / BuildingInstance.energy_ref_area,
            "LightingDemand_PE [kWh]": LightingDemand_PE_sum,
            "LightingDemand_PE [kWh/m2]": LightingDemand_PE_sum
            / BuildingInstance.energy_ref_area,
            ...
            "Appliance_gains_demand_GWP [kg/m2]": Appliance_gains_demand_Carbon_sum
            / BuildingInstance.energy_ref_area,
            "Appliance_gains_demand_PE [kWh]": Appliance_gains_demand_PE_sum,
            "Appliance_gains_demand_PE [kWh/m2]": Appliance_gains_demand_PE_sum
            / BuildingInstance.energy_ref_area,
            "GWP [kg]": Carbon_sum,
    ...
    remaining_calculation_time_2_saving_annualResults_summary = (
        remaining_buildings * time_building
    ) / 3600  # in Hours
    calculation_time_4_saving_hourlyResults = (
        length_iteration * time_building
    ) / 3600  # in Hours
    remaining_time_total = (
        remaining_calculation_time_2_saving_annualResults_summary
        + calculation_time_4_saving_hourlyResults
    )  # in Hours
    print("ETA:")
    print(
        "Estimated time for simulation and saving of annualResults_summary.xlsx",
        remaining_calculation_time_2_saving_annualResults_summary,
        "hours",
    )
    print(
        "Estimated time for simulation, saving of annualResults_summary.xlsx and saving of hourly results",
        remaining_time_total,
        "hours",
    )
...
################
annualResults_summary = pd.concat(list_of_summary)
annualResults_summary.to_excel(
    r"./results/annualResults_summary.xlsx", index=False)
################

print(
    "annualResults_summary.xlsx is now available in the DIBS---Dynamic-ISO-Building-Simulator\iso_simulator\annualSimulation\results folder"
)
print("Saving hourly results of each building to *BuildingID*.xlsx")
print(
    "This might take as long or longer than the simulation of the buildings before. You can savely abort the script if only the annualResults_summary.xlsx data is requiered."
)
...
print(
    "Simulation Completed. All saved results can be found in the DIBS---Dynamic-ISO-Building-Simulator\iso_simulator\annualSimulation\results folder"
)
