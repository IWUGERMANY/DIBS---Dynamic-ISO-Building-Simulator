...
def getGains(hk_geb, uk_geb, profile_from_norm, gains_from_group_values):
    ...
    gains_zuweisungen = pd.read_csv(os.path.join(
        '../auxiliary/norm_profiles/profiles_zuweisungen.csv'), sep=';', encoding='latin')
        ...
        if uk_geb in gains_zuweisungen['uk_geb'].values:
            ...
            if profile_from_norm == 'sia2024':
                ...
                gain_per_person = float(
                    row['gain_per_person_sia2024'].to_string(index=False).strip())
                ...
                gain_per_person = float(
                    row['gain_per_person_18599'].to_string(index=False).strip())
    ...
    if hk_geb in gains_zuweisungen['hk_geb'].values:
        ...
        if uk_geb in gains_zuweisungen['uk_geb'].values:
            ...
            if usage_from_norm == 'sia2024':
                usage_start = int(
                    row['usage_start_sia2024'].to_string(index=False).strip())
                usage_end = int(
                    row['usage_end_sia2024'].to_string(index=False).strip())

            elif usage_from_norm == 'din18599':
                usage_start = int(
                    row['usage_start_18599'].to_string(index=False).strip())
                usage_end = int(
